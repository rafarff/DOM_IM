...
<main class="container">
  ...
  <!-- TAB: DASHBOARD v7.1.0 — análise por bairro consolidada (bubble + timeline + 3 heatmaps) -->
  <div class="tab-panel" id="tab-dashboard">
    ...
    <section class="dash-section">
      ...
      <!-- Posicionamento competitivo: bubble ticket × R$/m² (cada bolha = 1 bairro, tamanho = nº empreend.) -->
      ...
      <!-- Lançamentos por bairro no tempo (semestre) -->
      <div class="dash-card-mini" style="margin-top:16px">
        <div class="chart-wrap" style="height:380px"><canvas id="ch-bairro-timeline"></canvas></div>
        <p class="dash-note" id="ch-bairro-timeline-note">Lançamentos por semestre, empilhados por bairro (top bairros · demais agregados em "Outros"). <strong>Atenção:</strong> datas com origem <em>T-36</em> (estimativa entrega-36m) aparecem em opacidade reduzida — não são datas reais e podem ter ±6 meses de erro.</p>
      </div>

      <!-- Heatmaps (consolidados de Mapas de Calor → todos com lente do bairro) -->
      <h3 class="dash-section-sub" style="margin-top:24px;color:var(--dom-gray-dark);font-size:14px">Cruzamentos do bairro</h3>
      <div class="dash-grid-half">
        <div class="dash-card-mini">
          <h3>Bairro × Incorporadora</h3>
          <div id="heatmap-bi" class="heatmap-host"></div>
          <p class="dash-note">Nº de empreendimentos por cruzamento. Mostra dominância territorial e onde há vácuo de competição.</p>
        </div>
        <div class="dash-card-mini">
          <h3>Bairro × Tipologia</h3>
          <div id="heatmap-bt" class="heatmap-host"></div>
          <p class="dash-note">Branco = vácuo de oferta. Hover mostra os empreendimentos.</p>
        </div>
        <div class="dash-card-mini">
...
function renderTable(data) {
  ...
  document.getElementById('tbody').innerHTML = completos.map(e => {
    ...
    const nomap = '';  // mapa removido v6.4 — badge sem propósito
    return `
  ...
  document.getElementById('tbody-b').innerHTML = incompletos.map(e => {
    const nomap = '';  // mapa removido v6.4
    const faltam = [];
...
// ═══════════════════════════════════════════════════════════════
// DASHBOARD v7.1.0 — análise por bairro reformada
//   · removidas as 2 barras antigas (VGV/m² por bairro) — redundantes com a tabela
//   · adicionado bubble ticket × R$/m² (cada bolha = bairro, tamanho = nº emp.)
//   · adicionado stacked bar de lançamentos por semestre × bairro (T-36 hachurado)
//   · 3 heatmaps consolidados na seção bairro:
//       Bairro × Incorporadora (novo) · Bairro × Tipologia · Bairro × Segmento
//   · seção "Mapas de Calor (cruzamentos)" foi removida (consolidação)
// ═══════════════════════════════════════════════════════════════
...
function renderDashBairros(data) {
  ...
  // ─── Timeline: lançamentos por semestre × bairro (top 6 bairros + Outros) ───
  // Constrói pares (semestre, bairro) com flag se a data é T-36 (estimativa).
  const lancamentos = data.filter(e => {
    const m = String(e.lancamento || '').match(/^(\d{1,2})\/(\d{4})$/);
    return m && e.bairro && e.bairro !== 'São Luís' && e.bairro !== 'Não identificado';
  });

  // Top bairros por nº de lançamentos (no recorte temporal já filtrado pelo dashboard)
  const bairroFreq = {};
  lancamentos.forEach(e => { bairroFreq[e.bairro] = (bairroFreq[e.bairro] || 0) + 1; });
  const TOP_N = 6;
  const topBairrosTime = Object.entries(bairroFreq).sort((a,b) => b[1]-a[1]).slice(0, TOP_N).map(x => x[0]);
  const bairroLabel = b => topBairrosTime.includes(b) ? b : 'Outros';

  // Agrupa por semestre (AAAA-Sx) × bairroLabel × origem (real | T-36)
  const semCounts = {};  // { 'AAAA-Sx': { bairroLabel: { real: n, t36: n } } }
  lancamentos.forEach(e => {
    const m = e.lancamento.match(/^(\d{1,2})\/(\d{4})$/);
    const mo = parseInt(m[1]); const yr = parseInt(m[2]);
    const sem = mo <= 6 ? 'S1' : 'S2';
    const key = yr + '-' + sem;
    const lbl = bairroLabel(e.bairro);
    const isT36 = (e.lancamento_origem || '').toUpperCase() === 'T-36';
    if (!semCounts[key]) semCounts[key] = {};
    if (!semCounts[key][lbl]) semCounts[key][lbl] = { real: 0, t36: 0 };
    semCounts[key][lbl][isT36 ? 't36' : 'real'] += 1;
  });

  const semestres = Object.keys(semCounts).sort();  // 'AAAA-Sx' já ordena cronologicamente
  const bairroOrder = topBairrosTime.concat(Object.values(semCounts).some(s => s['Outros']) ? ['Outros'] : []);

  // Paleta consistente para os bairros (ouro DOM como destaque + tons análogos)
  const bairroPalette = ['#C9A84C', '#1B4584', '#0F7B6C', '#8B2E2E', '#5D6E3C', '#6A4C93', '#8C8C8C'];
  const bairroColor = b => bairroPalette[bairroOrder.indexOf(b) % bairroPalette.length] || '#8C8C8C';

  destroyDashChart('ch-bairro-timeline');
  if (semestres.length > 0) {
    // Para cada bairro: 2 datasets (real, t-36) compartilhando a mesma stack
    const datasets = [];
    bairroOrder.forEach(b => {
      datasets.push({
        label: b + ' (data confirmada)',
        data: semestres.map(s => (semCounts[s][b]?.real) || 0),
        backgroundColor: bairroColor(b),
        borderColor: '#000', borderWidth: 0.5,
        stack: 'lancamentos'
      });
      datasets.push({
        label: b + ' (estimativa T-36)',
        data: semestres.map(s => (semCounts[s][b]?.t36) || 0),
        backgroundColor: bairroColor(b) + '55',  // alpha reduzido = ~33%
        borderColor: bairroColor(b),
        borderWidth: 1,
        borderDash: [4,3],
        stack: 'lancamentos'
      });
    ...
    DASH_CHARTS['ch-bairro-timeline'] = new Chart(document.getElementById('ch-bairro-timeline'), {
      type: 'bar',
      data: { labels: semestres, datasets: datasets },
      options: { responsive: true, maintainAspectRatio: false,
        plugins: {
          legend: { position: 'right', labels: { boxWidth: 12, font: {size: 10}, filter: (item) => !item.text.includes('estimativa T-36') } },
          title: { display: true, text: 'Empreendimentos lançados por semestre · empilhados por bairro' },
          tooltip: { callbacks: { label: ctx => ctx.dataset.label + ': ' + ctx.parsed.y } }
        },
        scales: {
          x: { stacked: true, title: { display: true, text: 'Semestre' } },
          y: { stacked: true, beginAtZero: true, ticks: { precision: 0 }, title: { display: true, text: 'Nº empreendimentos' } }
        }
      }
    });

    // Atualiza nota com diagnóstico de qualidade da data
    const totalReal = lancamentos.filter(e => (e.lancamento_origem || '').toUpperCase() !== 'T-36').length;
    const totalT36 = lancamentos.length - totalReal;
    const pctT36 = lancamentos.length ? Math.round((totalT36 / lancamentos.length) * 100) : 0;
    const noteEl = document.getElementById('ch-bairro-timeline-note');
    if (noteEl) {
      noteEl.innerHTML = 'Lançamentos por semestre, empilhados por bairro (top ' + TOP_N + ' · demais agregados em "Outros"). ' +
        '<strong>Qualidade da data:</strong> ' + totalReal + ' confirmadas · ' + totalT36 + ' estimativa T-36 (' + pctT36 + '%, em hachurado/translúcido — entrega menos 36 meses, ±6m de erro).';
    }
  }

  // ─── Heatmap Bairro × Incorporadora ───
  const allBairrosHM = [...new Set(data.map(e => e.bairro).filter(b => b && b !== 'São Luís' && b !== 'Não identificado'))].sort();
  const allIncs = [...new Set(data.map(e => e.incorporadora).filter(i => i))].sort();
  const matrixBI = {};
  allBairrosHM.forEach(b => { matrixBI[b] = {}; allIncs.forEach(i => matrixBI[b][i] = []); });
  data.forEach(e => {
    if (matrixBI[e.bairro] && matrixBI[e.bairro][e.incorporadora]) matrixBI[e.bairro][e.incorporadora].push(e);
  });
  const countsBI = allBairrosHM.flatMap(b => allIncs.map(i => matrixBI[b][i].length));
  const maxBI = countsBI.length > 0 ? Math.max(...countsBI, 1) : 1;

  let htmlBI = '<table class="heatmap-tbl"><thead><tr><th></th>';
  allIncs.forEach(i => htmlBI += '<th title="' + i + '">' + i.replace(/Engenharia/i, 'Eng.').replace(/Incorporação/i, 'Inc.') + '</th>');
  htmlBI += '</tr></thead><tbody>';
  allBairrosHM.forEach(b => {
    htmlBI += '<tr><td class="row-label">' + b + '</td>';
    allIncs.forEach(i => {
      const cell = matrixBI[b][i];
      const n = cell.length;
      const intensity = n > 0 ? Math.min(0.9, (n / maxBI) * 0.85 + 0.15) : 0;
      const bg = n > 0 ? 'rgba(201,168,76,' + intensity + ')' : '#fafafa';
      const tip = n > 0 ? cell.map(e => e.empreendimento).join(', ') : 'Sem oferta dessa incorporadora';
      const safe = tip.replace(/"/g, '&quot;');
      htmlBI += '<td style="background:' + bg + '" title="' + safe + '"><span class="cell-val">' + (n || '') + '</span></td>';
    });
    htmlBI += '</tr>';
  });
  htmlBI += '</tbody></table>';
  document.getElementById('heatmap-bi').innerHTML = allBairrosHM.length ? htmlBI : '<p style="color:var(--dom-gray-mid);font-size:12px;padding:20px">Sem dados nos filtros aplicados</p>';
}
