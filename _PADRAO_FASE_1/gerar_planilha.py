...
#         Book Nexus Coupé + 2 tabelas maio/2026 (visão multimodal): 152 unidades visíveis em
#         3 produtos Ergus — On Residence (90u 3D 3 plantas 80,38/81,21/95,84m²), Open Design
#         (62u Studio+1Q+2Q em 4 plantas: 30,79/33,51/45,20/59,30m²), Lead Office (corporativo,
#         fora de escopo IM). Rebrand "Nexus Coupé" Powered by Porsche Consulting. Endereço
#         atualizado: Av. Avicenia 15, Calhau (mantém Bairro=Renascença marketing). Tickets
#         R$ 535k–1.504k. R$/m² médio ~R$ 15.842 (LUXO). VGV est. ~R$ 164M. Lançamento 05/2026
#         (data tabela). Entrega 06/2031 (mais tardia entre 12/2030 Open + 06/2031 On Res).
#         U_RAW: unidades/Ergus__Nexus_Renascenca.yaml com 152 entries (orig=tabela_local_imagem,
#         status=disponível, estoque=1.0 por Rafael — pre-launch fase de reserva).
#         Removido de BREVE_LANCAMENTO_NAMES em build_panorama.py.
#     (B) MONTE MERU enriquecido com book digital (57p). Tipologia "—" → "3D" (3 suítes); 4 plantas
#         por andar confirmadas: APT01 141,36m² + APT02 135,77m² + APT03 135,32m² + APT04 135,83m²
#         (áreas E_RAW 135.32–141.36, era 135.32–135.83). Total inferido 40 unid (10 andares × 4
#         aptos, origem book §3.6 nível 3). Diferenciais: SPA Aquecido (1º de SLZ), piscina borda
#         infinita, Smart Home Alexa, energia solar, reuso água.
#   Total mapeado v11.21 → v11.22: Empreend 55 (=); Incorp 20 (=);
#     U_RAW arquivos/unid: 27/921 → 28/1.073 (+1 Nexus YAML +152 entries);
#     VGV total: R$ 3,37bi → ~R$ 3,53bi (+R$ 164M Nexus visível).
#   Drift script ↔ planilha esperado = 0 (a verificar). Invariantes §3.7.C.4 + C.6 a verificar.
#   Distribuição A/B/C: A +1 Nexus (28→29); B -1 Nexus (7→6); C (=).
#   INSIGHT estratégico DOM: Nexus Renascença = concorrente direto em vários segmentos. 3D
#     80–95m² Renascença R$ 1,0–1,5M vs Dom Lucas (3D ~100m² R$ 850k Cantinho do Céu) e Dom José
#     (4D 154m² R$ 1,4M). Open Design Studios + 1Q + 2Q ocupam nicho jovem profissional onde
#     DOM não atua. Em fase de reserva, 55+ studios já reservados antes do launch oficial.
#
...
#     • 2 books duplicados (md5 idêntico) → _DUPLICADAS_PRONTAS_PARA_DELETAR/
#     • 1 NOVA INCORPORADORA: BelSul Administração e Participações (20ª)
#         - Construtora: Eduardo Cardoso Engenharia
#         - Vendas: André Leite Imóveis (CRECI J-707)
#         - Arquitetura: Militão Gomes Arquitetura
#     • 1 NOVO empreend: Residencial Arizona (Alto do Calhau, 38 casas duplex 167,5m² 3 suítes)
#         - Tabela ABR/2026: 17 disp / 21 vend = 55% absorção
#         - Tickets R$ 1.711.788–1.746.369 (à vista) — R$/m² ~10.270 → Padrão Alto
#         - Entrega DEZ/2026 · VGV ~R$ 65,5M
#         - Diferenciais: energia solar, banda larga 3 anos, 3 estações carga elétrica
#     • Dom Lucas T18 (ABR/26) + T19 (MAI/26): tabelas movidas — re-extração diferida
#     • Dom José ABR rev3 + MAI/26: tabelas arquivadas — re-extração diferida
#   Total mapeado v11.14 → v11.15: Empreend 54 → 55 (+1); Incorporadoras 19 → 20 (+1);
#     U_RAW 26 → 27 arquivos / 905 → 943 unidades (+38); VGV ~R$ 3,29 bi → ~R$ 3,36 bi (+R$ 65,5M).

# v11.14 — (07/05/2026): UPDATE MÍNIMO — Tabela Zion Ponta d'Areia MAI/2026 confirma v11.13.
#   INBOX 07/05: 1 PDF processado (TABELA_ZION_PONTA_D_AREIA_052026.pdf, text-extraction).
#
#   ATUALIZAÇÕES:
#     • Zion Ponta d'Areia (Ergus): tabela MAI/2026 conferida via text-extraction.
#       Mesmas 10 unid disp + mesmas SKUs (T1/C1: 202, 1202; T1/C2: 1101, 1201, 1301, 1501;
#       T2/C2: 302, 1002, 1102, 1202). Mesmos preços à vista (R$ 2,17M-2,56M).
#       Mesmos R$/m² (14.610-17.213). Velocidade de vendas = 0 unid no mês ABR→MAI.
#       Único delta: re-parcelamento (14 mensais ABR/26 → 13 mensais MAI/26;
#       semestral OUT/26 → NOV/26 — efeito mecânico de 1 mês decorrido).
#       Política de preço inalterada (sem reajuste, sem desconto). Data extração
#       atualizada 02/05/2026 → 07/05/2026.
#     • Identificada (e mantida) divergência granular nos tickets D02-D09 do U_RAW v8.2
#       (visão multimodal) vs valores reais da tabela: agregado fica ~1% acima do
#       real (R$ 24,17M v8.2 vs R$ 23,93M tabela MAI/26). Como não há mudança no
#       perímetro (mesmas 10 disp, mesmo segmento Alto, mesmo R$/m² médio), opção
#       pelo update mínimo: corrige fonte+data+obs sem mexer nos tickets granulares.
#       Refactor completo dos tickets fica registrado como pendência futura.
#
#   IMPACTO NUMÉRICO: zero. Total mapeado v11.13 → v11.14 inalterado: 54 empreend,
#     19 incorporadoras, 100 linhas Composição, 3.415 unidades, R$ 3,29 bi VGV.
#     Cobertura U_RAW 25/27 = 93%. Aba Unidades 905. Drift script ↔ xlsx = 0.
#
#   ARQUIVO MOVIDO: _INBOX/Tabela Zion - Maio 2026.pdf →
#     03_ERGUS/ZION_PONTA_D_AREIA_122023/TABELA/TABELA_ZION_PONTA_D_AREIA_052026.pdf
#
# v11.13 — (06/05/2026): +3 incorporadoras NOVAS + +3 empreend NOVOS + Prime Cohama destravado
#                        + Monte Meru atualizado (tabela MAI/26) + Dom Antônio tickets reais.
#   INBOX 06/05: 9 arquivos PDFs processados (3 books + 5 tabelas + 1 livro Prime Cohama).
#
#   NOVAS INCORPORADORAS (decisão Rafael 06/05 — todas oficiais, sem distinção complementar):
#     • Áppia Construtora (Cond. Jardim di Vincenzo, Turu)
#     • ESA Empreendimentos (Fiji Residence, Cohama — Powered by HOUSI/RendMais)
#     • Amorim Coutinho (Cond. Essenza, Olho d'Água — upscale do MCMV pra luxo)
#
#   ATUALIZAÇÕES:
#     • Monte Meru (Berg): tabela MAI/2026 confere v11.12 (mesmas 2 disp + tickets idem). +obs
#       diferencial "1º andar com pé-direito de 4º pavimento + nascente total" via WhatsApp corretor.
#       Data extração 27/04 → 06/05/2026.
#     • Prime Cohama (MB Engenharia): DESTRAVADO via tabela ABR/26 + book. 22 casas mono 4 SUÍTES
#       159,99m² construída em lotes 218-312m². Tickets R$ 2,19M-2,5M (oferta promo Casa 10
#       R$ 1,97M com piscina+gourmet+terreno maior). 4 disp + 1 bloqueada + 17 vendidas.
#       Endereço: Estrada Velha do Pinheiro N 11. Entrega ABR/2030. Antes: "casas duplex 140m²"
#       (info errada de imprensa 2023 sem confirmação) → AGORA: "4 suítes 159,99m² + lazer".
#     • Dom Antônio (DOM): 12 contratos Clicksign processados (CASAS 01-12). Tickets reais
#       R$ 673.100,78 (Casa 03) a R$ 870.000 (Casa 10). Cronograma de vendas:
#       50% (6 casas) em ago/2023 [LANÇAMENTO real]; 5 casas em 2024; 1 em mar/2025.
#       Atualiza ticket único R$ 906.870 → range R$ 673.100-870.000 (origem informado_manualmente
#       via contratos). Mês de lançamento corrigido: 06/2023 → 08/2023.
#
#   NOVOS EMPREENDIMENTOS (3):
#     1. Cond. Jardim di Vincenzo (Áppia Construtora, Turu): 25 casas mono 97,93m² em lotes
#        variados 179-237m². Tickets R$ 656.131-694k (varia pelo tamanho do lote). Origem
#        tabela_local ABR/26. Status: 17 disp + 2 negoc + 6 vend (32% absorção).
#        Lançamento: ~03/2026 (PDF book criado em 17/03/26 — estimativa informado_manualmente).
#        Padrão Médio (R$/m² casa ~6,7-7,1k em ticket ~R$ 670k médio).
#     2. Fiji Residence (ESA Empreendimentos, Cohama Rua do Aririzal 85): 192 unid declaradas
#        oficial. 2 torres × 12 andares (sem 2º). 2D 61,79m² (1S+1Q) + 3D 80,53m² (1S+2Q).
#        Tickets R$ 657-1.018k (crescem por andar). Entrega DEZ/2029 (tabela construtora).
#        Lançamento JAN/2026 (imprensa blogsoestado 15/12/25). Origem tabela_local + imprensa.
#        Padrão Médio-Alto (blend ticket ~R$ 800k + R$/m² ~R$ 11k). Diferencial: 1º empreend.
#        de SLZ com APARTAMENTOS TOKENIZADOS (parceria HOUSI smart-living + RendMais).
#     3. Cond. Essenza (Amorim Coutinho, Olho d'Água): 10 casas alto-luxo 267m² 4 suítes
#        em condomínio fechado. Ticket único R$ 3.500.000 (a combinar). R$/m² R$ 13.108.
#        VGV R$ 35M. Lançamento MAI/2026 (informado WhatsApp corretora Sheila Cutrim/Habitare
#        — 4% comissão 50/50). Origem informado_manualmente (book não tem tabela formal).
#        Padrão Luxo (ticket > 2,5M). Diferenciais: piscina + gourmet privativos + academia
#        + salão festas + segurança/exclusividade. Movimento estratégico Amorim Coutinho upscale.
#
#   FONTES DESBLOQUEADAS:
#     • pendencias_TOTAL.md: -1 (Prime Cohama sai do tier 2 → Composição completa).
#
#   IMPACTO NUMÉRICO ESPERADO (a confirmar pós-execução):
#     • Empreend. mapeados: 51 → 54 (+3)
#     • Incorporadoras: 16 → 19 (+3)
#     • VGV total: ~R$ 3,03 bi → ~R$ 3,30 bi (+R$ 35M Essenza + ~R$ 162M Fiji + ~R$ 17M JdV
#       + ~R$ 49M Prime Cohama destravado +~R$ 0M Monte Meru update e Dom Antônio reset).
#
...
INCORPORADORAS = [
    ...
    "Áppia Construtora","ESA Empreendimentos","Amorim Coutinho",
    "BelSul"
]
...
E_RAW = [
    ...
    ("Ergus","Zion Ponta d'Areia",
     ...
     "https://www.ergus.com.br","07/05/2026",
     "Tipologia detalhada: 4 suítes + 3 vagas. 2 torres × 14 pavtos tipo × 2 aptos/andar (Coluna 1 + Coluna 2) = 56 aptos no edifício. **TABELA 04/2026 EXTRAÍDA VIA VISÃO MULTIMODAL (v8.2 — 02/05/2026)** a partir do PDF imagem (pdftoppm + Claude visão). Tabela mostra 10 unidades disponíveis: Torre 1 col 1: aptos 202, 1101, 1201, 1202, 1301, 1501 (R$2.170k-2.557k); Torre 2 col 1: aptos 302, 1102, 1202 (R$2.192k-2.445k). Todas 148,55m² (4D suítes uniforme). R$/m² 14.610-17.212 (média ~R$15.500). Estimativa total ~60 unid. % Vendido estimado 83%. Memorial Reg. nº 02, Matrícula 130.345, 1º Cartório SL. Nota da tabela: obra entregue DEZ/26 mas unidades vendidas após ABRIL/26 serão entregues JUN/27 — sinal de tração tardia. **TABELA 05/2026 (v11.14, 07/05/2026):** confirmou todas as 10 SKUs e preços via text-extraction. Velocidade de vendas = 0 unid no mês ABR→MAI. Política de preço inalterada (sem reajuste). Único delta: re-parcelamento mecânico (14→13 mensais; semestral OUT/26→NOV/26).", "book", None, "informado_manualmente"),

    ("Ergus","Nexus Renascença",
     "Av. Avicenia, 15, Calhau, São Luís - MA, 65071-370","Renascença",
     "Vertical",None,
     152,"05/2026","06/2031", 30.79,95.84,None, "Studio; 1D; 2D; 3D",
     535902,1503726, None,None, None,
     "tabela_local_imagem","tabela_local_imagem","tabela_local",
     "https://www.ergus.com.br","25/05/2026",
     "Tipologia detalhada: Studio + 1Q + 2Q + 3Q/3 suítes. Complexo Nexus Coupé (rebrand MAI/2026 com Powered by Porsche Consulting) = mixed-use Av. Avicenia 15 Calhau (Bairro mantido Renascença por marketing/nome do empreendimento — §3.10 nível 1 book). **Destravamento v11.22 (25/05/2026)** via 1 book + 2 tabelas maio/2026 (visão multimodal). Composto por 3 produtos Ergus: (1) **On Residence** — 15 pav × 6 aptos = 90 unid 3D, 3 plantas (80,38m² 3Q / 81,21m² 3Q / 95,84m² 3 suítes), tickets R$ 1,02–1,50M, entrega 06/2031. (2) **Open Design** — 14 pav × 9 aptos = 126 unid plan., 4 plantas (Studio 30,79m² + Studio 33,51m² + 1Q 45,20m² + 2Q 59,30m²), tickets R$ 0,54–1,06M, entrega 12/2030. (3) **Lead Office** (corporativo, fora do escopo IM residencial) — salas 175,15m² divisíveis. + Open Mall comercial. Tabela maio/2026 em 'fase de reserva' (pré-registro de incorporação) mostra 152 unidades em oferta: 90 On Res (todas disp, pré-launch) + 62 Open Des (estudios + 1Q parcialmente reservados). Estoque tratado como 1.0 (pré-launch — opção Rafael 25/05). VGV est. ~R$ 164M para residencial visível. R$/m² médio R$ 15.842 (Luxo). U_RAW completo em unidades/Ergus__Nexus_Renascenca.yaml (152 entries). Diferenciais comerciais: paisagismo como experiência urbana, Lead Office para corporativo, mall integrado. Porsche Consulting na gestão de processos. Saiu de BREVE_LANCAMENTO_NAMES (build_panorama).", "tabela_local_imagem", None, "informado_manualmente"),
    ...
    ("Berg Engenharia","Monte Meru",
     "Endereço não localizado, Península da Ponta D'Areia, São Luís - MA","Península",
     "Vertical",None,
     40,"04/2024","04/2027", 135.32,141.36,None, "3D",
     1932400,1944500, None,None,None,
     "tabela_local","tabela_local","imprensa",
     "https://www.bergengenharia.com.br","25/05/2026",
     'Tipologia detalhada: 3 SUÍTES + lavabo + WC serviço + até 3 vagas + depósito (Plus) + opções de planta (cozinha ampliada / 2 suítes + closet ampliado / grand home office). **Book digital Berg confirmou (v11.22, 25/05/2026):** 4 plantas por andar — APT01 141,36m² + APT02 135,77m² + APT03 135,32m² + APT04 135,83m². Tipologia E_RAW corrigida "—" → "3D" (3 suítes contam). Tabela MAI/2026 confirma 2 unid. disponíveis: apto 103 (135,32m² APT03, R$ 1.932.400) + apto 104 (135,83m² APT04, R$ 1.944.500). 9 unidades VENDIDAS (apto 204–1004 = posição 04 floors 2–10). Posições 01 e 02 (APT01 141,36m² + APT02 135,77m²) inferidas como 100% vendidas (não aparecem na tabela disp). Total inferido = 10 andares × 4 aptos = 40 unid (origem book — descrição arquitetônica + numeração). Lançamento 04/2024 estimado pela pasta. Conclusão: 30/04/2027 (T-36 perfeito). **Diferenciais comerciais:** vista permanente mar OU lagoa, todos aptos nascente, Smart Home (Alexa + biometria + tomadas USB), 1º SPA AQUECIDO de SLZ, piscina com borda infinita, energia solar (~2500 kWh/mês cobertura), reuso de água condensadores, infra carros elétricos. Apto 704 tem 3 vagas (diferencial). 1º andar disp tem pé-direito de 4º pavimento + nascente total (WhatsApp corretor 06/05/2026). Correção INCC. Histórico Berg 28 anos: Montparnasse, Golden Tower, Peninsula Mall, Monte Olimpo, Monte Fuji.', "book", None, "informado_manualmente"),
    ...
    ("MB Engenharia","Condomínio Prime Cohama",
     "Estrada Velha do Pinheiro N 11, Cohama, São Luís - MA","Cohama",
     "Horizontal",None,
     22,"01/2026","04/2030", 159.99,159.99,None, "4D",
     2190000,2500000, None,None, 4/22,
     "tabela_local","tabela_local","imprensa",
     "https://www.instagram.com/mbengenharia.br/","06/05/2026",
     "Tipologia detalhada: 22 casas mono 4 SUÍTES, 159,99 m² construída em lotes 218,78-312,29 m² (variação só pelo tamanho do lote). Endereço: Estrada Velha do Pinheiro N 11. Entrega ABR/2030 (em construção). Tabela ABR/2026 (Tabela Bancos): tickets R$ 2.190.000 (Casas 10, 21) | R$ 2.400.000 (Casa 15) | R$ 2.500.000 (Casa 22). Avaliação CEF R$ 2.105.000, financiamento 90% R$ 1.894.500. **Casa 10 em PREÇO DE OPORTUNIDADE R$ 1.971.000** (com piscina + espaço gourmet + terreno maior que padrão). Status (implantação book): 4 disponíveis (Casas 10, 15, 21, 22) + 1 BLOQUEADA + 17 VENDIDAS = 22 total → estoque 4/22 ≈ 18%. Construção e Incorporação: MB Engenharia. **CORREÇÃO v11.13**: cadastro anterior dizia 'casas duplex 140m²' (info de imprensa 2023 sem confirmação) — book + tabela 2026 confirmam casas com 4 SUÍTES 159,99m² (não duplex sem tipologia), produto horizontal alto/luxo (R$/m² R$ 13.690-15.625).", "tabela_local", None, "imprensa"),

    ("DOM Incorporação","Dom Antônio",
     "Endereço não localizado, Jardim Eldorado (Turú), São Luís - MA","Jardim Eldorado",
     "Horizontal",None,
     12,"08/2023","—", 136.2,136.2,None, "3D",
     673100,870000, None,None, 0.0,
     "informado_manualmente","informado_manualmente","interno",
     "https://www.imovelnacidade.com/destaque/mb-construtora/","06/05/2026",
     "Tipologia detalhada: 3 suítes casas duplex. DOM Incorporação com MB Engenharia como sócia (empreendimento conjunto). **TOTAL = 12 casas mono 3D 136,2 m²** (mono-tipologia, padrão duplex idêntico). **TICKETS REAIS via 12 contratos Clicksign processados v11.13 (06/05/2026):** range R$ 673.100,78 (Casa 03) a R$ 870.000,00 (Casa 10), médio ~R$ 745k — variação pela data de assinatura (correção monetária + ajuste de timing). **Cronograma de vendas confirmado pelos contratos:** 6 casas (50%) assinadas em 10/08/2023 [→ LANÇAMENTO real ago/2023, corrige 06/2023 anterior]; 5 casas em 2024 (Casas 01, 02, 04, 08, 12); 1 casa em mar/2025 (Casa 10). Composição via §3.7 nível 5.1 (mono): 12u 3D 136,2m². **100% VENDIDO** (estoque=0, última venda 31/03/2025). Promovido para Tabela A v11.8. NOTA privacidade: dados pessoais dos compradores (nomes/CPF/RG/endereços residenciais) NÃO foram persistidos — só dados comerciais agregados (tickets + datas de assinatura).", "informado_manualmente", "informado_manualmente", None),
    ...
    # ═══ v11.13 — 3 NOVOS EMPREENDIMENTOS (06/05/2026) ═══════════════════════════════
    # Decisão Rafael 06/05: cadastrar 3 incorporadoras NOVAS como oficiais (todas
    # entram na lista monitorada, sem distinção complementar — vide memória
    # feedback_lista_incorporadoras_unica.md).

    # ─── ÁPPIA CONSTRUTORA — Cond. Jardim di Vincenzo (Turu) ─────────────────
    ("Áppia Construtora","Condomínio Jardim di Vincenzo",
     "Endereço a confirmar, Turu, São Luís - MA","Turu",
     "Horizontal",None,
     25,"03/2026","—", 97.93,97.93,None, "—",
     656131,693868, None,None, None,
     "tabela_local","tabela_local","informado_manualmente",
     "https://www.instagram.com/apiaengenharia/","06/05/2026",
     "Tipologia detalhada (Tabela ABR/2026 + Book Áppia 17/03/2026): 25 casas mono 97,93 m² construída em LOTES variados 179,34-236,93 m² (variação de ticket é função do lote, casa idêntica). Distribuição: Quadra 01 = Casas 01-12 (12u), Quadra 02 = Casas 13-25 (13u). Status (tabela ABR/26): 17 disponíveis + 2 em negociação + 6 vendidas = 32% absorção (8/25). Tickets R$ 656.131,12 (Casa 01) a R$ 693.868,12 (Casa 12) — financiamento 36x R$ 2.278-2.409/mês após sinal+anuais+chave. Lazer privativo: piscina + área gourmet em cada casa. Lançamento estimado mar/2026 (book criado 17/03/26 — informado_manualmente). Padrão Médio (R$/m² casa ~6,7-7,1k). NOVA INCORPORADORA cadastrada v11.13. Bairro Turu confirmado Rafael 06/05.", "tabela_local", None, "informado_manualmente"),

    # ─── ESA EMPREENDIMENTOS — Fiji Residence (Cohama) ────────────────────────
    ("ESA Empreendimentos","Fiji Residence",
     "Rua do Aririzal, 85, Cohama, São Luís - MA","Cohama",
     "Vertical",None,
     192,"01/2026","12/2029", 61.79,80.53,None, "2D; 3D",
     657681,1018448, None,None, None,
     "tabela_local","tabela_local","imprensa",
     "https://www.instagram.com/esa.empreendimentos/","06/05/2026",
     "Tipologia detalhada (Tabela MAI/2026 BL_AB + CONSTRUTORA + Book ESA dez/2025): TOTAL DECLARADO oficial = 192 unid em 2 torres (A,B) × 12 andares úteis (sem 2º). 2 plantas: TIPO 02 = 61,79 m² (1 suíte + 1 dormitório, 2D — banheiro social + banheiro suíte), TIPO 01 = 80,53 m² (1 suíte + 2 dormitórios, 3D — banheiro social + banheiro suíte + varanda suíte). Tickets: R$ 657.681 (61,79m² 5º andar Pos 2) a R$ 1.018.448 (80,53m² 12º andar Pos 1) — crescem por andar e posição. Entrega DEZ/2029 (tabela construtora). Lançamento JAN/2026 (imprensa blogsoestado 15/12/2025: 'ESA Empreendimentos lança Fiji Residence'). Endereço: Rua do Aririzal 85 Cohama. **Diferencial estratégico:** primeiro empreend. de São Luís com APARTAMENTOS TOKENIZADOS (parceria HOUSI smart-living + RendMais — gestão de locação, comunicação portaria, agenciamento serviços, mercado integrado). Algumas unidades em azul na tabela são tokenizadas + mobiliadas pela HOUSI. Lazer: piscinas adulto+infantil, multi-sport, gym, gourmet, churrasqueira, kiosk, party room, brinquedoteca, playground, pet play, spa+sauna. Padrão estimado Médio-Alto (blend ticket ~R$ 800k Médio + R$/m² ~R$ 11-13k Alto). NOVA INCORPORADORA cadastrada v11.13.", "tabela_local", None, "imprensa"),

    # ─── AMORIM COUTINHO — Cond. Essenza (Olho d'Água) ────────────────────────
    ("Amorim Coutinho","Condomínio Essenza",
     "Endereço a confirmar, Olho d'Água, São Luís - MA","Olho d'Água",
     "Horizontal","Luxo",
     10,"05/2026","—", 267,267,None, "4D",
     3500000,3500000, None,None, 1.0,
     "informado_manualmente","informado_manualmente","informado_manualmente",
     "https://amorimcoutinho.com.br","06/05/2026",
     "Tipologia detalhada (Book Essenza Luxury Living jan/2026 + WhatsApp corretora Sheila Cutrim Habitare 06/05/2026): 10 casas mono 267 m² em condomínio fechado de alto padrão. **4 SUÍTES por casa** (incluindo suíte master com closet). 2 pavimentos (Piso Térreo + Piso Superior). Lançamento MAI/2026. Olho d'Água. Ticket R$ 3.500.000 (forma de pagamento 'a combinar'). R$/m² R$ 13.108. VGV calculado: 10 × R$ 3,5M = R$ 35M. Padrão Luxo (ticket > R$ 2,5M). Diferenciais: design contemporâneo + acabamentos refinados, área de lazer privativa por casa (espaço gourmet + piscina), estrutura coletiva com academia + salão de festas, segurança e exclusividade pela baixa densidade (10 unid). **Movimento estratégico:** Amorim Coutinho é tradicionalmente top 15 nacional MCMV (Popular/Médio) — Essenza é primeira incursão da marca em Luxo. Imobiliária parceira: Habitare Imóveis (consultora Sheila Cutrim 98 98864-4053, comissão 4% 50/50 corretor/imobiliária). NOVA INCORPORADORA cadastrada v11.13 — Rafael 06/05 confirmou (texto WhatsApp original grafou 'Almorim Coutinho' mas grafia oficial é Amorim Coutinho).", "informado_manualmente", "informado_manualmente", "informado_manualmente"),

    # ─── BELSUL — Residencial Arizona (Alto do Calhau) ────────────────────────
    ("BelSul","Residencial Arizona",
